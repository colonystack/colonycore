...
func TestLineRequiresGenotypeMarkers(t *testing.T) {
	store := NewStore(nil)
...
func TestPermitRequiresRequiredArrays(t *testing.T) {
	store := NewStore(nil)
	ctx := context.Background()
	now := time.Now()
	if _, err := store.RunInTransaction(ctx, func(tx domain.Transaction) error {
...
func TestProjectRequiresFacilities(t *testing.T) {
	store := NewStore(nil)
	ctx := context.Background()
	if _, err := store.RunInTransaction(ctx, func(tx domain.Transaction) error {
...
func TestSupplyRequiresFacilitiesAndProjects(t *testing.T) {
	store := NewStore(nil)
	ctx := context.Background()
	if _, err := store.RunInTransaction(ctx, func(tx domain.Transaction) error {
		facility, err := tx.CreateFacility(domain.Facility{Name: "Facility"})
...
func TestSampleRequiresChainOfCustody(t *testing.T) {
	store := NewStore(nil)
	ctx := context.Background()
	now := time.Now()
	if _, err := store.RunInTransaction(ctx, func(tx domain.Transaction) error {
