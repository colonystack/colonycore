...
func TestMemoryStoreFullCrudCoverage(t *testing.T) {
	...
	if _, err := store.RunInTransaction(ctx, func(tx domain.Transaction) error {
		marker, err := tx.CreateGenotypeMarker(domain.GenotypeMarker{Base: domain.Base{ID: "marker-full"}, Name: "Marker", Locus: "loc", Alleles: []string{"A"}, AssayMethod: "PCR", Interpretation: "ctrl", Version: "v1"})
		if err != nil {
			return err
		}

		line, err := tx.CreateLine(domain.Line{Base: domain.Base{ID: "line-full"}, Code: "L", Name: "Line", Origin: "field", GenotypeMarkerIDs: []string{marker.ID}})
		if err != nil {
			return err
		}
		strain, err := tx.CreateStrain(domain.Strain{Base: domain.Base{ID: "strain-full"}, Code: "S", Name: "Strain", LineID: line.ID, GenotypeMarkerIDs: []string{marker.ID}})
		if err != nil {
			return err
		}
		...
		housing, err := tx.CreateHousingUnit(domain.HousingUnit{Base: domain.Base{ID: "housing-full"}, Name: "Housing", FacilityID: facility.ID, Capacity: 2, Environment: domain.HousingEnvironmentAquatic})
		if err != nil {
			return err
		}
		...
		protocol, err := tx.CreateProtocol(domain.Protocol{Base: domain.Base{ID: "protocol-full"}, Code: "PROT", Title: "Protocol", MaxSubjects: 5})
		if err != nil {
			return err
		}
		...
		cohort, err := tx.CreateCohort(domain.Cohort{Base: domain.Base{ID: "cohort-full"}, Name: "Cohort", Purpose: "Study", ProjectID: &project.ID, HousingID: &housing.ID, ProtocolID: &protocol.ID})
		if err != nil {
			return err
		}

		breeding, err := tx.CreateBreedingUnit(domain.BreedingUnit{Base: domain.Base{ID: "breeding-full"}, Name: "Breeding", Strategy: "pair", LineID: &line.ID, StrainID: &strain.ID})
		if err != nil {
			return err
		}
		...
		organism, err := tx.CreateOrganism(domain.Organism{Base: domain.Base{ID: "organism-full"}, Name: "Org", Species: "Spec", LineID: &line.ID, StrainID: &strain.ID, CohortID: &cohort.ID, HousingID: &housing.ID})
		if err != nil {
			return err
		}
		...
		procedure, err := tx.CreateProcedure(domain.Procedure{Base: domain.Base{ID: "procedure-full"}, Name: "Proc", Status: domain.ProcedureStatusScheduled, ScheduledAt: now, ProtocolID: protocol.ID, CohortID: &cohort.ID, OrganismIDs: []string{organism.ID}})
		if err != nil {
			return err
		}
		...
		treatment, err := tx.CreateTreatment(domain.Treatment{Base: domain.Base{ID: "treatment-full"}, Name: "Treat", Status: domain.TreatmentStatusPlanned, ProcedureID: procedure.ID, OrganismIDs: []string{organism.ID}, CohortIDs: []string{cohort.ID}})
		if err != nil {
			return err
		}
		...
		observation, err := tx.CreateObservation(domain.Observation{Base: domain.Base{ID: "observation-full"}, ProcedureID: &procedure.ID, OrganismID: &organism.ID, RecordedAt: now, Observer: "tech"})
		if err != nil {
			return err
		}
		...
		permit, err := tx.CreatePermit(domain.Permit{Base: domain.Base{ID: "permit-full"}, PermitNumber: "PERMIT", Authority: "Gov", ValidFrom: now, ValidUntil: now.Add(time.Hour), AllowedActivities: []string{"store"}, FacilityIDs: []string{facility.ID}, ProtocolIDs: []string{protocol.ID}})
		if err != nil {
			return err
		}
		...
		supply, err := tx.CreateSupplyItem(domain.SupplyItem{Base: domain.Base{ID: "supply-full"}, SKU: "SKU", Name: "Item", QuantityOnHand: 1, Unit: "unit", FacilityIDs: []string{facility.ID}, ProjectIDs: []string{project.ID}})
		if err != nil {
			return err
		}
