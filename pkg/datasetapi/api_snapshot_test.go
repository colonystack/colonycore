...
// This test enforces the exported surface of datasetapi against a committed
// snapshot (see internal/ci/datasetapi.snapshot). It provides two modes:
...
//   go test ./pkg/datasetapi -run TestDatasetAPISnapshot
...
var updateSnapshot = flag.Bool("update", false, "update datasetapi snapshot")

const snapshotFileName = "datasetapi.snapshot"

// TestGenerateDatasetAPISnapshot regenerates the snapshot when -update is supplied.
func TestGenerateDatasetAPISnapshot(t *testing.T) {
	if !*updateSnapshot {
...
// TestDatasetAPISnapshot compares the live surface with the committed snapshot.
func TestDatasetAPISnapshot(t *testing.T) {
	path := resolveSnapshotPath(t)
...
func currentAPISnapshot(t *testing.T) ([]byte, error) {
	...
	for _, name := range scope.Names() {
		...
		default:
			// Basic / defined types (e.g., type Dialect string)
	...
	buf.WriteString("# Generated snapshot of exported datasetapi surface (types, funcs, consts, vars, methods on exported interfaces) used by TestDatasetAPISnapshot.\n")
	for _, l := range lines {
		buf.WriteString(l)
		buf.WriteByte('\n')
	}
...
func resolveSnapshotPath(t *testing.T) string {
	...
	// When tests run inside pkg/datasetapi, walk up until we see internal/ci.
	dir := wd
